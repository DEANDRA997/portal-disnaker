...
import { 
  ...
  Download, Search, UserPlus, Users, Trash2, Key 
} from 'lucide-react';

import { getTransactions, saveTransaction, getWarehouseStock, getActiveUser, logoutUser, getAllUsers, createUser, resetUserPassword, deleteUser } from '../actions';
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-white/5 backdrop-blur-md border border-white/10 p-3 px-5 rounded-2xl shadow-lg">
            <div className={`p-2 rounded-full ${isAdmin ? 'bg-rose-500/20 border border-rose-500/50' : isMenteriOrWamen ? 'bg-amber-500/20 border border-amber-500/50' : 'bg-slate-700'}`}>
              {isAdmin ? <ShieldAlert className="w-5 h-5 text-rose-400 animate-pulse" /> : isMenteriOrWamen ? <ShieldCheck className="w-5 h-5 text-amber-400" /> : <User className="w-5 h-5 text-slate-300" />}
            </div>
            <div className="flex flex-col pr-3 border-r border-white/10">
              <p className={`text-sm font-bold ${isAdmin ? 'text-rose-400' : isMenteriOrWamen ? 'text-amber-400' : 'text-white'}`}>{activeUser.name}</p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            <div className="bg-emerald-900/10 p-5 rounded-2xl border border-emerald-500/20">
              ...
              <p className={`text-3xl font-bold ${summaryKasGov >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{summaryKasGov >= 0 ? `+$${summaryKasGov.toLocaleString('en-US')}` : `-$${Math.abs(summaryKasGov).toLocaleString('en-US')}`}</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 bg-[#151822]/80 backdrop-blur-md p-6 rounded-2xl border border-white/5 shadow-xl h-fit">
              ...
              <table className="w-full text-left border-collapse min-w-max relative">
                <thead className="sticky top-0 bg-[#151822] z-10 shadow-sm">
                  <tr className="text-[10px] uppercase tracking-widest text-slate-500 font-bold border-b border-white/5">
                    ...
                    <th className="p-4">Info Borongan Barang</th>
                ...
                <tbody className="text-sm">
                  {isLoading ? (
                    ...
                    <tr><td colSpan={6} className="p-8 text-center text-slate-500">Tidak ada transaksi ditemukan.</td></tr>
                  ) : (
